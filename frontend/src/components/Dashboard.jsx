...
function FileUpload({ onUploadSuccess, dataStatus, authFetch }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      const result = await authFetch('/data/upload', { method: 'POST', file });
      if (result?.success) {
        onUploadSuccess?.();
      } else {
...
export default function Dashboard() {
  ...
  // AUTH: Fetch wrapper that adds Authorization header
  ...
  const fetchAllData = useCallback(async (nextMode = mode) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const [statusRes, summaryRes, timelineRes, spendingRes, subsRes, anomaliesRes] = await Promise.all([
        authFetch(`/data/status${q}`),
        authFetch(`/summary${q}`),
        authFetch(`/timeline${q}`),
        authFetch(`/insights/spending${q}`),
        authFetch(`/insights/subscriptions${q}`),
        authFetch(`/insights/anomalies${q}`)
      ]);
  ...
  const handleReset = async () => {
    await authFetch('/data/reset', { method: 'POST' });
    fetchAllData(mode);
  };

  const handleGoalCalculate = async (amount, months) => {
    const result = await authFetch('/insights/goal', {
      method: 'POST',
      body: { goal_amount: amount, goal_months: months, mode }
    });
    setGoalData(result);
  };
